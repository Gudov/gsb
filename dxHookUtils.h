...
float* worldview;
float* proj;
std::vector<int> goodView;

DirectX::XMMATRIX statProj{
			1, 0, 0, 0,
			0, 1.78f, 0, 0,
			0, 0, -1.9f, -1,
			0, 0, 0, 0
...
std::vector<AsteroidSubData> asteroidsSubData;
...
void AddModel(ID3D11DeviceContext* pContext)
{
	...
	if (m_pCurWorldViewCB != nullptr && m_pCurProjCB != nullptr)
	{
		...
		if (goodView.empty()) {
			...
			for (int i = 0; i < 100; i++) {
				if (cmpf(view[i + 5], 1, 0.01) && cmpf(view[i + 10], 1.78f, 0.05)) {
					goodView.push_back(i);
				}
			}
		}
	...
	if (method4 == 1)
	{
		...
		DirectX::XMMATRIX viewProjMatrix = DirectX::XMMatrixMultiply((FXMMATRIX)*matWorldView, statProj);//multipication order matters
...
{
	ofstream fout;
	fout.open(GetDirFile((char*)"w2sf.ini"), ios::trunc);
...
void LoadCfg()
{
	ifstream fin;
	string Word = "";
	fin.open(GetDirFile((char*)"w2sf.ini"), ifstream::in);
	fin >> Word >> wallhack;
	fin >> Word >> chams;
	fin >> Word >> esp;
	fin >> Word >> aimbot;
...
static void ShaderHook(ID3D11Device* device, const char* type, const void** bytecode, SIZE_T* bytecode_size)
{
	...
	//{
		//Log("2");
	//MessageBoxA(nullptr, "dumpShader", "dumpShader", MB_OK);
	...
	{
		//MessageBoxA(nullptr, "fake", "fake", MB_OK);
		//Log("5");
